...
from processing.grouped_batch_generator import GroupedBatchGenerator, GroupedBatchGenerator2
...
def run(batch_size: int=128,
    ...
    for index, (train, val) in enumerate(iterations):
        ...
        trainStream = GroupedBatchGenerator(train, featureBuilder, neg_ratio, batch_size, min_group)
        valStream = GroupedBatchGenerator(val, featureBuilder, neg_ratio, batch_size, min_group)

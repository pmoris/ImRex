...
def run(batch_size: int=512,
    ...
    for index, (train, val) in enumerate(iterations):
        ...
        if negative_path:
            ...
            trainStream = PPILitGenerator2(train, negTrain, neg_ratio, batch_size, pepRange, pepRange, swap=swap)
            valStream = PPILitGenerator2(val, negVal, neg_ratio, batch_size, pepRange, pepRange, swap=swap)

        else:
            print("train set", len(train))
            print("val set", len(val))

            trainStream = PPILitGenerator(train, neg_ratio, batch_size, pepRange, pepRange)

...
from src.models.modelPPIPadded import ModelPPIPadded
...
def run(
    ...
    model = ModelPPIPadded(
        max, max, nameSuffix=name, channels=featureBuilder.getNumberLayers()
    )
    ...
    for index, (train, val) in enumerate(iterations):
        ...
        if negative_path:
            ...
            trainStream = PaddedBatchGenerator2(
                train,
                negTrain,
                featureBuilder,
                neg_ratio,
            ...
            valStream = PaddedBatchGenerator2(
                val,
                negVal,
                featureBuilder,
                neg_ratio,
